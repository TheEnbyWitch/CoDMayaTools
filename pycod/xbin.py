...
class XBlock(object):
    ...
    def WriteBoneInfoBlock(file, bone_index, bone):
        ...
        name_size = len(name)
        data = struct.pack('Hxxii%ds' % padded(name_size + 1), 0xF099, bone_index, bone.parent, str(name))
        file.write(data)
    ...
    def WriteMaterialInfoBlock(file,
        ...
        strings = (str(__str2bytes__(material.name)),
                   str(__str2bytes__(material.type)),
                   str(__str2bytes__(serialize_image_string(
    ...
    def WritePartInfo(file, index, name):
        XBlock.WriteMetaObjectInfo(file, 0x360B, index, str(name))
